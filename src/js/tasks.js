...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (localStorage.length) {
    for (let i = 0; i < localStorage.length; i++) {
      ...
      block.innerHTML = `
        ...
        <a class="tasks__item-link"><img class="tasks__item-img" src="${details}"></a>
        ...
        <p>Прогресс цели: ${sumArrItems(obj.itemPayments)} ${rightSumFormat1(obj.itemPayments)} из ${obj.itemGoalSum} ${rightSumFormat2(obj.itemGoalSum)}</p>
        <div class="progress">
  ...
  function countDays(date1, date2) {
    ...
    let text =  endNum === '1' ? "день" :
              (endNum === '2' || endNum === '3' || endNum === '4') ? "дня" :
              "дней";
  ...
  function rightSumFormat1(item) {
    ...
    let text =  endNum === '1' ? "рубль" :
              (endNum === '2' || endNum === '3' || endNum === '4') ? "рубля" :
              "рублей";
    return text;
  }
